...
export function generateTemplateMaterialsForProject(project: ProjectItem): ProjectMaterialRequirementItem[] {
  ...
  return templates.map((tpl, idx) => {
    ...
    return {
      ...
      projectName: project.name,
      customerName: project.customer || 'Direct Customer',
      poNumber: project.poNumber || '-',
      poDate: project.startDate || new Date().toISOString().split('T')[0],
      machineType: project.machineType || project.name || 'General Machine',
      machineName: project.machineName || project.machineType || project.name || 'General Machine',
      date: project.date || new Date().toLocaleDateString('en-GB'),
      poNo: project.poNo || project.poNumber || '-',
      vendorName: project.vendorName || project.vendor || 'Standard Vendor',
      orderedBy: project.orderedBy || 'Admin',
      ...
      vendor: project.vendor || tpl.defaultVendor || 'Standard Vendor',
      bomRef: project.projectNumber ? `BOM-${project.projectNumber}` : `BOM-${(project.machineType || 'GEN').replace(/\s+/g, '').substring(0, 6)}`,
      ...
      outsourcingCost: 0,
      totalCost,
      notes: `${project.machineType || project.name} assembly component`,
    };
  });
}
