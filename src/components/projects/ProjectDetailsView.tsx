...
export const ProjectDetailsView: React.FC<ProjectDetailsViewProps> = ({
  ...
  return (
    <div className="space-y-6 select-text relative">
      {/* Toast Notification for DB Save Confirmation */}
      {saveSuccessMessage && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-slate-900 border border-emerald-500 text-white shadow-2xl animate-bounce">
          <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0" />
          <div className="text-xs">
            <span className="font-black text-emerald-300 block">Verified & Saved</span>
            <span className="text-slate-300">{saveSuccessMessage}</span>
          </div>
        </div>
      )}
